...
export default function HeroBanner({ 
  ...
  return (
    <div className="relative w-full min-h-[75vh] md:min-h-[85vh] flex items-end pb-16 pt-24 overflow-hidden">
      {/* Background Backdrop */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="max-w-2xl space-y-3.5">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs font-semibold">
            <span className={`px-2.5 py-0.5 rounded text-white font-bold tracking-wider uppercase text-[10px] ${
              current.industry === 'Bollywood' ? 'bg-amber-600' : 'bg-blue-600'
            }`}>
              {current.industry === 'Bollywood' ? 'Bollywood 🇮🇳' : 'Hollywood 🇺🇸'}
            </span>

            <span className="px-2 py-0.5 rounded bg-white/[0.08] text-gray-200 border border-white/10 text-[11px] font-mono">
              4K UHD
            </span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-semibold text-emerald-400 bg-emerald-950/60 border border-emerald-500/30 px-2 py-0.5 rounded flex items-center gap-1">
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1 text-xs text-gray-400">
            <span className="text-gray-300 font-semibold">Director: {current.director}</span>
            <span>•</span>
            <div className="flex flex-wrap gap-1.5">
              {current.genres?.map(genre => (
                <span 
                  key={genre} 
                  className="px-2 py-0.5 rounded bg-white/[0.05] text-gray-300 border border-white/10"
                >
                  {genre}
                </span>
              ))}
          ...
          <div className="flex flex-wrap items-center gap-3 pt-3">
            {/* Watch Now - Solid Netflix style */}
            <button
              onClick={() => onPlay(current)}
              className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white hover:bg-gray-200 text-black font-extrabold shadow-xl hover:scale-105 active:scale-95 transition-all text-sm"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-3 rounded-xl bg-white/15 hover:bg-white/25 text-white font-semibold backdrop-blur-md border border-white/15 transition-all text-sm"
            >
            ...
            <button
              ...
              className="p-3 rounded-xl bg-white/10 hover:bg-white/20 text-gray-300 hover:text-white border border-white/10 transition-all"
      ...
      <div className="absolute right-4 md:right-8 bottom-10 z-20 flex items-center gap-2">
        <button
          onClick={prevSlide}
          className="p-2 rounded-full bg-black/60 hover:bg-white/20 text-white border border-white/10 transition-all"
          ...
          {featuredItems.map((item, idx) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                idx === currentIndex ? 'w-6 bg-[#e50914]' : 'w-2 bg-white/30 hover:bg-white/50'
              }`}
            />
          ))}
        </div>
